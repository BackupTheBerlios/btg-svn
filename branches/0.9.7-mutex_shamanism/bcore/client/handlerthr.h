         ...
         namespace client
            {
               ...
               class handlerThreadIf : 
                  ...
                  public:
                     ...
                     /// Release and lock the thread's mutex
                     /// used to temporarily return control to another thread
                     void triggerThreadMutexLock();
                  ...
                  private:
                     /// mutex lock of owner's thread
                     boost::mutex::scoped_lock* threadMutexLock;
                  };

               /// Class to pass thread's mutex::lock to handlerThreadIf
               class scopedLockPasser
                  {
                     /// pointer to handlerThreadIf::threadMutexLock instance we're working with
                     /// can be NULL
                     boost::mutex::scoped_lock** pthreadMutexLock;
                  public:
                     scopedLockPasser(handlerThreadIf & htif, boost::mutex::scoped_lock & lock);
                     virtual ~scopedLockPasser();
                  };
