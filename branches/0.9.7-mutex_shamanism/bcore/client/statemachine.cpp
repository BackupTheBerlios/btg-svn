...
#define DEFAULT_ERROR_ACTION                                            \
         ...
         bool stateMachine::receiveCommand(Command* & _target, bool const _waitforever)
         {
            ...
            dBuffer dbuffer;
#if BTG_STATEMACHINE_DEBUG
            BTG_NOTICE(logWrapper(), "receiveCommand(_waitforever=" << _waitforever << ")");
#endif // BTG_STATEMACHINE_DEBUG
...
BEGIN_READ:
            ...
            if (!read_status)
               {
                  // ask toplevel what to do
                  if (clientcallback->onReadTimeout())
                  {
                     handlerThreadIf * phtif = dynamic_cast<handlerThreadIf*>(clientcallback);
                     if (phtif)
                     {
                        // unlock and lock mutex to temporarily return control to another thread
                        phtif->triggerThreadMutexLock();
                     }
         ...
         {
#if BTG_STATEMACHINE_DEBUG
            BTG_NOTICE(logWrapper(), "step_SM_COMMAND_ACK_WAIT(): counter_ack=" << counter_ack <<", counter_ack_max=" << counter_ack_max);
#endif // BTG_STATEMACHINE_DEBUG
